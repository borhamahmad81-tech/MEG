...
class BrowserController:
    ...
    def navigate_to_dashboard(self, base_url, country, form_id, timeout=25):
    ...
    # ------------------------------------------------------------------ #
    def get_record_rows(self, timeout=25):
    ...
    def open_record(self, row_element, timeout=15):
    ...
    def _fill_field(self, key, text, timeout=10):
        el = self.find(key)
    ...
    def click_save(self, timeout=15):
        sel = self.selectors["save_button"]
        last_exc = None
        for attempt in range(3):
            try:
                btn = WebDriverWait(self.driver, timeout).until(
                    EC.element_to_be_clickable((self._by(sel), sel["value"]))
                )
                try:
                    btn.click()
                except Exception:
                ...
                last_exc = exc
                self.logger(f"Save button not clickable yet (attempt {attempt + 1}), retrying...", "warn")
                time.sleep(1.5)
        raise last_exc
