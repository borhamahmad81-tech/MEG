...
class MatchedRule:
    ...
    def __init__(self, idx, row, matched_keywords):
        self.idx = idx
        self.row = row
    ...
    def __repr__(self):
        return f"<MatchedRule {self.rule_id} ({self.event_code}) kw={self.matched_keywords}>"
...
class RulesEngine:
    ...
    def find_matching_rule(self, event_code: str, free_text: str):
        """
        Scans every subcategory row for this Event_Code and returns a
        MatchedRule for the row with the most keyword hits, or None if no
        row's keywords matched at all (-> Component D: human review).
        """
