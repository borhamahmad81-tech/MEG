...
class AutomationWorker(threading.Thread):
    def __init__(self, settings, log_queue: queue.Queue,
        ...
        self.default_used = DeferredQueue(settings.resolve_path("default_used_path"))

        self.counts = {"processed": 0, "deferred": 0, "blocked": 0, "skipped": 0, "default_used": 0}
    ...
    def _final_report(self, heading: str):
        ...
        lines = [
            ...
            f"  of which used a DEFAULT (no keyword match, review these): {c['default_used']}",
    ...
    def _process_single_record(self, record):
        ...
        if matched_rule is None:
            # Only reachable if the event code itself somehow has zero rows
            # (shouldn't happen since has_rule() already passed above).
            self.log(
        ...
        if matched_rule.is_default_fallback:
            self.log(
                f"[Default Used] ID: {record_id} ({event_code} / {matched_rule.rule_id}): "
                f"no subcategory keyword matched -- used the default row. Review this one "
                f"afterward (see default_used_records.json) to confirm it's correct.",
                "warn",
            )
            self.default_used.add(record_id, event_code, free_text, reason="default_fallback")
            self.counts["default_used"] += 1
            self.push_status()
