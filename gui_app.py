...
class SafetyEventAssistantApp(ctk.CTk):
    ...
    def _build_webscrape_tab(self, parent):
        ...
        ctk.CTkLabel(logic_frame, text="Page/Element Wait Timeout (seconds) -- increase "
                                        "if your connection or the site is slow").pack(anchor="w", padx=12)
        self.page_wait_var = tk.StringVar(value=str(self.settings.get("page_wait_timeout", 25)))
        page_wait_entry = ctk.CTkEntry(logic_frame, textvariable=self.page_wait_var)
        page_wait_entry.pack(anchor="w", padx=12, pady=(0, 10), fill="x")
        page_wait_entry.bind("<FocusOut>", lambda e: self._save_page_wait_timeout())
        ...
        self.required_status_var = tk.StringVar(value=self.settings.get("required_status"))
        self._labeled_entry(file_frame, "Required Status (only process this stage)",
                             self.required_status_var, "required_status")
        self.required_centre_var = tk.StringVar(value=self.settings.get("required_centre"))
        self._labeled_entry(file_frame, "Required Centre (blank = all)",
                             self.required_centre_var, "required_centre")
    ...
    def _open_code_picker(self):
        ...
        # One BooleanVar per code, reused across search-filter rebuilds so
        # checked state is never lost while typing in the search box.
        check_vars = {
            code: tk.BooleanVar(value=(code in currently_selected) or not currently_selected)
            for code in codes
        }

        search_var = tk.StringVar(value="")
        search_entry = ctk.CTkEntry(modal, textvariable=search_var,
                                     placeholder_text="Search codes... (e.g. A.14)")
        search_entry.pack(padx=12, pady=(0, 6), fill="x")

        scroll = ctk.CTkScrollableFrame(modal, width=380, height=300)
        ...
        def rebuild_list(*_):
            ...
            for code in codes:
                if query and query not in code.lower():
                    continue
                ctk.CTkCheckBox(scroll, text=code, variable=check_vars[code]).pack(anchor="w", pady=2)
        ...
        # Paste a list of codes (comma/space/newline separated) instead of
        # clicking through checkboxes one by one.
        ctk.CTkLabel(modal, text="Or paste a code list (comma/space/newline separated):").pack(
            anchor="w", padx=12, pady=(6, 2)
        )
        paste_box = ctk.CTkTextbox(modal, height=60)
        paste_box.pack(padx=12, pady=(0, 6), fill="x")

        def apply_pasted_list():
            raw = paste_box.get("1.0", "end")
            pasted_codes = {c.strip() for c in re.split(r"[,\s]+", raw) if c.strip()}
            if not pasted_codes:
                return
            matched = 0
            for code, var in check_vars.items():
                if code in pasted_codes:
                    var.set(True)
                    matched += 1
                else:
                    var.set(False)
            self._log(f"Applied pasted list: {matched} of {len(pasted_codes)} pasted code(s) matched.", "info")

        ctk.CTkButton(modal, text="Apply Pasted List (replaces current selection)",
                      command=apply_pasted_list).pack(padx=12, pady=(0, 10), fill="x")
        ...
        def save_and_close():
            ...
            modal.grab_release()
            modal.destroy()
            self._log(f"Code selection updated and saved: {len(selected)} code(s) selected.", "info")

        ctk.CTkButton(modal, text="Save", fg_color="#2E8B57",
                      command=save_and_close).pack(padx=12, pady=(0, 12), fill="x")
    ...
    def _save_page_wait_timeout(self):
        try:
            self.settings.set("page_wait_timeout", int(self.page_wait_var.get()))
        except ValueError:
            pass
    ...
    def _launch_browser_debug(self, browser):
        ...
        if not exe:
            ...
            if not picked:
                vendor_folder = "Google\\Chrome" if browser == "chrome" else "Microsoft\\Edge"
                messagebox.showerror(
                    "Launch failed",
                    f"Could not find {browser.title()} automatically, and no file was selected. "
                    f"It's usually at C:\\Program Files\\{vendor_folder}\\Application\\.",
                )
                return
            exe = picked
    ...
    def _start_assistant(self):
        ...
        confirmed = messagebox.askyesno(
            ...
            "Before continuing:\n\n"
            "1. Is the debug browser window (launched via 'Launch Chrome/Edge (debug)') "
            "open and showing the 'All Safety Events' dashboard table?\n"
            "2. Has the page fully finished loading (no spinner, rows visible)?\n\n"
            "Click Yes only if both are true. If you just launched the browser, "
            "click No, log in, navigate to the dashboard, wait for it to load, "
            "then click Start Assistant again.",
        )
        if not confirmed:
            self._log("Start cancelled -- confirm the dashboard is loaded first.", "warn")
            return
